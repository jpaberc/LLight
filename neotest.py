...
def fill(strip):
	for i in range(strip.numPixels()):
		strip.setPixelColor(i, Color(52,255,0))
...
def colorSampler(strip):
	for i in range(128):
		for j in range(64):
			for k in range(strip.numPixels()):
				strip.setPixelColor(k, Color(i, 255, j))
			strip.show()
			time.sleep(0.1)
